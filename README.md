...
This module comprises **EmbeddableLogRecord** and **LogRecordFormatParser** classes. Its purpose is to implement a message template as described on [messagetemplates.org](https://messagetemplates.org/). The main goal is to encourage the generation of human readable logs in addition to the generation of structured logs. 

It works by receiving a log message template in the same format as that described on the Message Templates site, using *LogRecordFormatParser* class. It also requires a tree data structure such as the ones used by **TreeDataMatcher** module, and a list of positional arguments. Using this three inputs, **EmbeddableLogRecord** class can generate both structured and unstructured strings, where the structured ones require a serialization implementation (e.g to JSON).

*Key differences in implementation with those on the Message Templates site are that positional arguments are distinct from keyword arguments; and positional and keyword arguments can be present in a template at the same time. A "hole" which is an integral index refers to a positional argument; otherwise it refers to a node in the tree data structure input. Also alignment and format options ar not supported.*
...
to generate a single *EmbeddableLogRecord* subclass instance. Assuming the subclass was implemented to yield JSON, then calling toStructuredLogRecord() on the instance yields an object whose toString() in turn gives
...
On the other hand, calling toLogFormatString(List) returns this format string, assuming subclass was implemented to use SLF4J string format syntax.
```
