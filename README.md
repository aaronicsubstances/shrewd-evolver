...
This module comprises **LogMessageTemplate** and **LogMessageTemplateParser** classes. It implements a message template as described on [messagetemplates.org](https://messagetemplates.org/). Its main goal is to encourage the generation of human readable logs in addition to the generation of structured logs. 

It works by receiving a log message template in the same syntax as that described on the Message Templates site, using *LogMessageTemplateParser* class. It also requires a tree data structure supplying keyword arguments such as the ones used by **TreeDataMatcher** module, and a list of positional arguments. Using this three inputs, **LogMessageTemplate** class can generate both structured and unstructured strings, where the structured ones require a serialization implementation (e.g to JSON).

*Although syntax is the same, semantics is quite different. These are the key semantic differences with that specified on the Message Templates:* 

   1. Positional arguments are supplied as a list, and are separate from keyword arguments which is supplied as a single tree data structure.
   1. Positional and keyword arguments can be present in a template at the same time. A replacement field (aka "hole" in Message Templates terminology) which is a valid integral index refers to a positional argument; otherwise it refers to a child node in the keyword arguments tree data structure. Negative positional indices are accepted and behave like Python negative indexing of arrays.
   1. A keyword can be a complex path referring to any node in the supplied keyword arguments tree data structure. 
   1. Capturing rules and alignment and format options are not supported.
