...
This module comprises **EmbeddableLogRecord** and **LogRecordFormatParser** classes. Its purpose is to support *LogNavigation* module by encouraging the generation of human readable logs even if structured logs are the ones actually used during testing. By generating and associating unstructured (but human readable) logs with structured ones, this module increases the likelihood that a future maintainer of the software project will understand the structured logs. This module thus serves as  a means to program comprehension for software maintenance and evolution.

It works by receiving a log message template independent of any logging library specifics, using *LogRecordFormatParser* class. It also requires a tree data structure such as the ones used by **TreeDataMatcher** module. Using this two inputs, **EmbeddableLogRecord** class can generate both structured and unstructured strings, where the structured ones require a serialization implementation (e.g to JSON).
